...
def unet2d_full(
    ...
    model.compile(optimizer=Adam(learning_rate=lr), loss=lossfunc, metrics=[metric])
...
def unet3d_big_expectedvalue(
    ...
    model.compile(
        optimizer=Adam(learning_rate=lr), loss=lossfunc, metrics=metric, loss_weights=loss_weights
    )
...
def unet3d_big_1cam(
    ...
    model.compile(optimizer=Adam(learning_rate=lr), loss=lossfunc, metrics=["mse"])
...
def remove_heatmap_output(model, params):
    ...
    if "heatmap_output" in lay:
        ...
        opt = Adam(learning_rate=float(params["lr"]))
